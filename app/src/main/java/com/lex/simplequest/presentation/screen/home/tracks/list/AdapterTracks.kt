        ...
        fun bind(track: Track) {
            viewBinding.apply {
                ...
                if (distance >= Config.METERS_IN_KILOMETER) {
                    format =
                        "%s, %.2f " + context.resources.getString(R.string.others_km) + if (Config.DEBUG) ", ${track.checkPoints.size} / ${track.points.size}" else ""
                    distance /= Config.METERS_IN_KILOMETER
                } else {
                    format =
                        "%s, %.2f " + context.resources.getString(R.string.others_m) + if (Config.DEBUG) ", ${track.checkPoints.size} / ${track.points.size}" else ""
                }
